...
from app.services.meget.config import BASE_URL as MEGET_BASE_URL
from app.services.bon_ua import scrape_bon_ua_listing, get_listing_urls as bon_ua_get_listing_urls
from app.services.bon_ua.config import BASE_URL as BON_UA_BASE_URL
...
def scrape_meget_command(workers, pages):
    click.echo(f"🚀 Starting Meget scraping with {workers} threads, {pages} pages from {MEGET_BASE_URL}...")

    all_target_urls = set()
    for page in range(1, pages + 1):
        click.echo(f"[CRAWLER] Page {page}...")
        urls = meget_get_listing_urls(base_url=MEGET_BASE_URL, page=page)
        if urls:
...
def scrape_bon_ua_command(workers, pages):
    click.echo(f"🚀 Starting Bon.ua scraping with {workers} threads, {pages} pages from {BON_UA_BASE_URL}...")

    all_target_urls = set()
    for page in range(1, pages + 1):
        click.echo(f"[CRAWLER] Page {page}...")
        urls = bon_ua_get_listing_urls(listings_url=BON_UA_BASE_URL, page=page)
        if urls:
            all_target_urls.update(urls)
        time.sleep(1)
